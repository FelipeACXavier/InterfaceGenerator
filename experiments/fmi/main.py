...
def main():
  ...
  if server_generator:
    server_result = server_generator.generate(output_name, config)

    if not server_result.is_success():
      print(f'Failed to generate server: {server_result}')

  if client_generator:
    client_result = client_generator.generate(output_name, config)

    if not client_result.is_success():
      print(f'Failed to generate client: {client_result}')
